...
function add_params()
	...
	for i = 1,16 do
  	...
  	params:add{type="control", id="cutoff" .. i, name="cutoff " .. i, controlspec=cs_CUT, action=function(x) set_filt_cutoff(i - 1, x) end}
		...
		cs_GAIN = controlspec.new(0.0,4.0,'lin',0.0,0.2,'')
...
function set_filt_gain(synth_num, value)
	engine.filt_gain(synth_num, value)
	edit = synth_num
	redraw()
end

function set_filt_cutoff(synth_num, value)
	engine.filt_cutoff(synth_num, value)
	edit = synth_num
	redraw()
end
...
function enc(n, delta)
	...
	elseif n == 3 then
		...
		elseif key_1_pressed == 0 and key_2_pressed == 0 and key_3_pressed == 1 then
			...
			params:set("cutoff" .. edit+1, params:get("cutoff" .. edit+1) + delta * 100)
...
function redraw()
	...
	screen.level(15)
	screen.text(" ")
	screen.text(params:get("cutoff" .. edit+1)/1000 .. " kHz")
	screen.move(0,19)
	screen.level(2)
